...
const Card = ({ data, media }) => {
  ...
  const toggleFavorite = async () => {
    if (data) {
      ...
      favorito ?   await removeToFavorites(data.id || data._id) : await addFavorites(data,media)
    }
  };
  ...
  const toggleVerDespues = async () => {
    if (data) {
      setVer(!verDespues);
      verDespues ? await removeToWatchLater(data.id || data._id) : await addWatchLater(data,media);
    }
  };
  ...
  const toggleVistas = async () => {
    if (data) {
      setVistas(!vistas);
      vistas ? await removeToWatched(data.id || data._id) : await addWatched(data,media);
    }
  };
